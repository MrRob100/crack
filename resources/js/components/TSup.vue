        ...
        load: function() {
            ...
            request.onload = function() {
                ...
                isso.ctx.decodeAudioData(audioData, function(buffer) {
                    ...
                    //connect convolver stuff
                    source.connect(convolverGain);
                    source2.connect(convolverGain);
                    source.connect(masterGain);
                    source2.connect(masterGain);
                    
                    //going to have to reconnect using other gains
                    ...
                    // filter.connect(isso.ctx.destination); //without 
                    gainNode.connect(filter).connect(isso.ctx.destination);
                    gainNode2.connect(filter).connect(isso.ctx.destination);

                    //init gain vals for 0 phaser
                    gainNode.gain.value = 0;
                    gainNode2.gain.value = -1;
            ...
            request.send();
            getImpulse(convolverGain, masterGain);

            function getImpulse(convolverGain, masterGain) {
                //convolver meat n potatos ()
                var iconvolver = isso.ctx.createConvolver();
                var ajaxRequest = new XMLHttpRequest();
                var impulseUrl = '/crack/public/storage/data/tenniscourt.wav';
                ...
                ajaxRequest.onload = function() {
                    ...
                    isso.ctx.decodeAudioData(impulseData, function(buffer) {
                        ...
                        iconvolver.buffer = myImpulseBuffer;
                        iconvolver.loop = true;
                        iconvolver.normalize = true;

                        //was prefixed with i to diferentiate
                        convolverGain.gain.value = 0;
                        convolverGain.connect(iconvolver);
                        iconvolver.connect(masterGain);


                        //while not propperly plumbed in
